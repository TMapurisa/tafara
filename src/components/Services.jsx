...
const Services = () => {
  ...
  return (
    <section id="services" className="bg-bg-secondary py-[100px] px-6">
      <div className="max-w-[1140px] mx-auto">
        <div className="text-center mb-[50px] reveal">
          <h1 className="font-headings text-[3.4rem] md:text-[4rem] font-bold text-text-primary tracking-tight mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {serviceData.map((service) => (
            <div 
              key={service.id} 
              className="bg-bg-card border border-border-color rounded-xl p-[32px_28px] hover:-translate-y-1 hover:border-border-hover hover:shadow-[0_16px_48px_rgba(0,0,0,0.25)] transition-all duration-300 ease-custom reveal"
            ...
            >
              ...
              <h2 className="font-headings text-[1.9rem] font-semibold text-text-primary mb-3">
                {service.title}
              </h2>
              <p className="text-[1.5rem] text-text-secondary leading-[1.65]">
                {service.desc}
              </p>
            </div>
          ))}
